...
test('buildOfflineDataPlan uses pinned cache exchange before lab runtime exchange', () => {
  const plan = buildOfflineDataPlan({
    pinnedData: { enabled: true, cacheRoot: '/cache', exchangeName: 'ccxt-exchange' },
    labs: [{ labId: 'primary', symbol: 'XRPUSDT', timeframe: '15m', limit: 1000, when: '2026-04-21T00:00:00Z', exchange: 'default_exchange' }],
    offline: { mode: 'offline-strict' },
  });

  assert.equal(plan.requiredLabs[0].exchangeName, 'ccxt-exchange');
});
