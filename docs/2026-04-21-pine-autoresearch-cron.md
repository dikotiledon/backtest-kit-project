...
- Current emergency local-only operating mode trims the matrix to XRPUSDT only and anchors `when` to the latest proven local cache tail when full cross-symbol cache coverage is unavailable.
