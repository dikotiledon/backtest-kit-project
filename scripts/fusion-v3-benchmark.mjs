...
import { applyPatchPlan, buildPatchPlan, cartesianProduct, configIdFromCombo, rankSweepResults, leaderboardMarkdown } from './lib/pine-tuner.mjs';

const cwd = process.cwd();
const inputPath = path.resolve(cwd, 'pine/test.pine');
const source = await fs.readFile(inputPath, 'utf8');
const when = '2026-04-21T10:30:00.000Z';
const runId = 'fixed-window-fusion-v3-XRPUSDT-15m-10000-2026-04-21T10-30-00Z';
...
const combos = cartesianProduct(grid).map((item) => ({
  ...control,
...
const configs = [{ label: 'control', combo: control }, ...combos.map((combo, idx) => ({ label: `fusion-v3-${String(idx + 1).padStart(2, '0')}`, combo }))];
const results = [];
...
for (let index = 0; index < configs.length; index++) {
  ...
  const configId = item.label === 'control' ? item.label : configIdFromCombo(index, item.combo);
