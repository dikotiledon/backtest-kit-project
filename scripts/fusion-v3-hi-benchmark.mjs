...
import { applyPatchPlan, buildPatchPlan, configIdFromCombo, rankSweepResults, leaderboardMarkdown } from './lib/pine-tuner.mjs';

const cwd = process.cwd();
const inputPath = path.resolve(cwd, 'pine/test.pine');
const source = await fs.readFile(inputPath, 'utf8');
const when = '2026-04-21T10:30:00.000Z';
const runId = 'fixed-window-fusion-v3-hi-XRPUSDT-15m-10000-2026-04-21T10-30-00Z';
...
function baseControl(minPredSum) {
  return {
    ...
    fusionMaxBonus: 0.5,
    fusionPenaltyPerMissing: 0.5,
  };
}

const thresholdBand = [2.0, 2.5, 3.0];
const penaltyBand = [0.5, 1.0, 1.5];
const confirmFamilies = [
  { family: 'atr-only', useAtrFlipConfirm: true, useEngulfingConfirm: false, useEmaCrossConfirm: false },
  { family: 'atr-engulf', useAtrFlipConfirm: true, useEngulfingConfirm: true, useEmaCrossConfirm: false },
  { family: 'atr-ema', useAtrFlipConfirm: true, useEngulfingConfirm: false, useEmaCrossConfirm: true },
  { family: 'atr-engulf-ema', useAtrFlipConfirm: true, useEngulfingConfirm: true, useEmaCrossConfirm: true },
];

const configs = [
  ...thresholdBand.map((minPredSum) => ({ label: `control-${minPredSum.toFixed(1)}`, combo: baseControl(minPredSum) })),
  ...thresholdBand.flatMap((minPredSum) =>
    penaltyBand.flatMap((fusionPenaltyPerMissing) =>
      confirmFamilies.map((family) => ({
        label: `fusion-v3-hi-${minPredSum.toFixed(1)}-${family.family}-p${fusionPenaltyPerMissing.toFixed(1)}`,
        combo: {
          ...baseControl(minPredSum),
          useSignalFusion: true,
          useFusionV3: true,
          useAtrFlipConfirm: family.useAtrFlipConfirm,
          use3LineConfirm: false,
          useEngulfingConfirm: family.useEngulfingConfirm,
          useEmaCrossConfirm: family.useEmaCrossConfirm,
          fusionBonusPerSignal: 0.25,
          fusionMaxBonus: 0.5,
...
for (let index = 0; index < configs.length; index++) {
  ...
  const configId = configIdFromCombo(index, item.combo);
